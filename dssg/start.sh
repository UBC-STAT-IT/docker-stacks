...
if [ "$(id -u)" == 0 ] ; then
    ...
    if [ "${NB_UID}" != "$(id -u "${NB_USER}")" ] || [ "${NB_GID}" != "$(id -g "${NB_USER}")" ]; then
        ...
        # Create local groups
        for gid in $(echo $GROUPS_GIDS | tr -d "[],'");
        do
            if [ $gid != "${NB_GID}" ]; then
                groupadd --force --gid $gid --non-unique "${NB_USER}"
            fi
        done

        useradd --no-log-init --home "/home/${NB_USER}" --shell /bin/bash --uid "${NB_UID}" --gid "${NB_GID}" --groups 100,$(echo $GROUPS_GIDS | tr -d "[]' ") "${NB_USER}"
    fi
